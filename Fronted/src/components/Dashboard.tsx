...
const DIFFICULTY_CLASSES: { [key: string]: string } = {
  adiestrado: 'bg-info',
  regular: 'bg-warning text-dark',
  principiante: 'bg-danger',
};
...
const Dashboard: React.FC = () => {
  ...
  const loadDashboardData = useCallback(async (): Promise<void> => {
    try {
      ...
      const [tasksData, statsData] = await Promise.all([
        taskAPI.getTasks(),
        isAdminUser ? taskAPI.getStatistics() : Promise.resolve(null),
      ]);

      const sortedRecentTasks = tasksData
  ...
  const getDifficultyClass = useCallback((difficulty: string): string => 
    DIFFICULTY_CLASSES[difficulty] || 'bg-secondary',
  []);
  ...
  const truncateDescription = useCallback((description: string, maxLength: number = 50): string =>
      ...
      {isAdminUser && (
        <div className="row mb-4">
          {stats.map((stat, index) => (
            <div key={`${stat.title}-${index}`} className="col-xl-3 col-md-6 mb-4">
                    ...
                    <tbody>
                      {recentTasks.map(task => (
                        <tr key={task.id}>
                          ...
                          <td>
                            <span className={`badge ${getDifficultyClass(task.difficulty)}`}>
                              {task.difficulty}
                            </span>
                          </td>
